...
<p align="center">
  ...
  <a href="#-retrieval-engine">Retrieval</a> •
  <a href="#-wiki-engine">Wiki</a> •
...
![Search Papers + Wiki overview](assets/search-papers-wiki-en.png)

## What Is Search Papers + Wiki?

**Search Papers + Wiki** is an AI-native research workflow for people who do not want paper search, PDF storage, reading notes, and knowledge graphs to live in four different places.

It has two connected halves:

- **Search Papers, the retrieval side**: a Rich-powered academic paper CLI that rewrites Chinese or English research topics into precise English search terms, searches arXiv/CrossRef/Scopus/PubMed in parallel, enriches results, ranks papers, and generates structured AI reading reports.
- **Wiki Bridge + Wiki, the knowledge side**: a semi-automatic pipeline that turns selected papers into PDF files, metadata stubs, LLM prompts, bidirectional links, and an Obsidian-ready Markdown knowledge graph.

The result is a one-stop academic operating system: **search faster, read deeper, think further, connect everything**.

## Why It Stands Out

| Usual workflow | Search Papers + Wiki |
|----------------|----------------------|
| Search databases one by one | Searches 4 academic APIs concurrently with independent timeout handling |
| Save random PDFs and loose notes | `ingest <N>` creates a PDF + Markdown source stub in a versioned Wiki |
| Trust a black-box summary | DeepSeek produces structured reports, single-paper reading guides, and multi-paper comparisons |
| Lose context after reading | Entities, concepts, sources, synthesis notes, and explorations become a linked knowledge graph |
| Keep raw files and generated notes mixed together | `raw/` stays read-only; `wiki/` is LLM-managed and Obsidian-compatible |

## Retrieval Engine

The CLI is designed for serious literature triage:

- **AI query rewriting**: Chinese or English topics are rewritten by DeepSeek into comma-separated academic English search terms; multi-word phrases are quoted automatically.
- **Parallel retrieval**: `asyncio.gather` queries arXiv, CrossRef, Scopus, and PubMed concurrently. Each source has independent timeout and failure handling.
- **Result enrichment**: merged papers are deduplicated, then Semantic Scholar backfills missing citation counts and author information.
- **Transparent ranking**: papers are scored by relevance (50%), citation count (25%), recency (15%), and source quality (10%), then mapped to a 1-5 star recommendation index.
- **AI reports**: after search, DeepSeek generates a Markdown report with a field overview, Top 5 recommendations, subtopic clusters, trends, and next-step suggestions.
- **Detail mode**: `detail <N>` produces both a basic paper explanation and a tailored reading guide; `detail 1 3 5` adds cross-paper comparison.
- **Deep search**: `depth >= 2` follows DOI reference chains through CrossRef, deduplicates new papers, and can recurse up to 5 levels.
- **PDF downloader**: arXiv/direct links first, DOI-to-publisher HTML parsing second, Unpaywall OA third, and optional Selenium + Firefox fallback for stricter publisher pages.

## Wiki Engine

`wiki_bridge.py` is the bridge from retrieval to durable knowledge:

- `ingest <N>` first runs duplicate checks: exact DOI, exact title, then fuzzy title match with `SequenceMatcher >= 85%`.
- It generates a stable source filename such as `EFM-2025.md`, with automatic suffixes for conflicts.
- It downloads the PDF into `wiki/raw/literature/` while keeping `raw/` read-only by convention.
- It writes a source stub into `wiki/sources/literature/` with frontmatter, PDF link, metadata table, abstract, and fill-in sections for goals, methods, findings, value, data availability, entities, and concepts.
- `build_agent_prompt()` prepares the LLM handoff: the agent reads PDFs, fills the source page, adds formulas/tables, updates entity and concept backlinks, refreshes `wiki/index.md`, and logs work in `wiki/log.md`.
- The Wiki follows a five-layer Obsidian-compatible structure: **Entities**, **Concepts**, **Sources**, **Synthesis**, and **Explorations**.

This is why the project is not just a paper search script. It is a workflow for growing a research graph that can answer compound questions like: *what coupling mechanism links a material, a structure, and a failure mode?*

## Features

| Area | Highlights |
|------|------------|
| Retrieval | arXiv, CrossRef, Scopus, PubMed, async parallel search, citation-chain depth search |
| AI analysis | query optimization, field reports, Top 5 recommendations, single-paper guides, multi-paper comparison |
| Ranking | relevance/citation/recency/source-quality weighted scoring with 1-5 star recommendations |
| PDF access | direct PDF, DOI HTML parsing, publisher-specific handling, Unpaywall, Selenium fallback |
| Wiki | metadata stubs, LLM handoff prompts, bidirectional links, Obsidian-ready Markdown graph |
| Terminal UX | Rich tables, panels, Markdown rendering, command-driven workflow |
