...
**Search Papers** 是一个终端运行的学术文献搜索工具，能同时查询 **arXiv、CrossRef、Scopus、PubMed** 四个数据库，并利用 **DeepSeek AI** 对搜索结果进行智能分析、排序和解读。支持递归引用追踪（深度搜索）和通过校园网/VPN 自动下载论文 PDF。
...
- **Wiki 知识库集成** — 论文摄入生成元数据桩，可对接 LLM agent 完善笔记
...
5. ingest 3                             # 下载 PDF + 保存元数据
6. ask 这个领域有哪些未解决的问题         # 自由提问
```
